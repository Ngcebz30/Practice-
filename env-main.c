...
int main(void) {
    ...
    while (1) {
        ...
        if (strcmp(args[0], "exit") == 0) { // check if the user entered the exit command
            break; // exit the shell by breaking out of the while loop
        }
        else if(strcmp(args[0], "env") == 0) { // check if the user entered the env command
            char **env = environ; // create a local copy of the environment variables
            while (*env) { // loop through all the environment variables
                printf("%s\n", *env++); // print out each variable
            }
